...
use JHtml;
use JText;
use RegularLabs\BetterPreview\Params;
...
class Preview extends Helper
{
	...
	function render(&$article, $context)
	{
		...
		RL_Document::script('betterpreview/preview.min.js', '6.0.0');
		RL_Document::style('betterpreview/preview.min.css', '6.0.0');
		...
		foreach ($data as $key => $val)
		{
			...
			if (in_array($key, ['published', 'state', 'access', 'tags', 'hits', 'version', 'created', 'modified', 'publish_up', 'publish_down']))
			{
				continue;
			}
